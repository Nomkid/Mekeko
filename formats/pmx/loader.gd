...
const STANDARD_MAGIC = [0x50, 0x4D, 0x58, 0x20]
const LOCKED_MAGIC = [0x50, 0x4D, 0x58, 0x20] # TODO: find out what those "edit locking" programs do to this file magic

func _get_recognized_extensions():
	return PackedStringArray(["pmx"])

func _get_resource_script_class(path: String):
	return ""

func _get_resource_type(path: String):
	var magic := _check_file_magic(FileAccess.open(path, FileAccess.READ))
	if magic == Magic.STANDARD or magic == Magic.LOCKED:
		return "PackedScene"
	else:
		return ""
...
func _load(path: String, original_path: String, use_sub_threads: bool, cache_mode: int):
	...
	var idxmap := {}
	var skeleton := PMXSkeleton3D.new()
...
func _check_file_magic(file: FileAccess) -> Magic:
	var magic := file.get_buffer(4)
	if magic == PackedByteArray(STANDARD_MAGIC):
		return Magic.STANDARD
	elif magic == PackedByteArray(LOCKED_MAGIC):
		return Magic.LOCKED
	else:
